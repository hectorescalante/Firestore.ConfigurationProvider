...
using Firestore.ConfigurationProvider;
...
namespace Tests.UnitTests
{
  public class ApplicationSettingsManagerTests
  {
    ...
    [Fact]
    public async Task TestLoadDocumentSettingsOnChangeAsync_WithMachineDocument_ShouldSuccess()
    {
      //Arrange
      var provider = _autoFixture.Create<FirestoreConfigurationProvider>();
      ...
      connectionMock
        .Setup(mock => mock.GetConfigurationDocumentLevels())
        .Returns(new List<ConfigurationLevels>() { { ConfigurationLevels.Application }, { ConfigurationLevels.Stage }, { ConfigurationLevels.Machine } });
      connectionMock
        .Setup(mock => mock.GetDocumentFieldsAsync(It.Is<ConfigurationLevels>(param => param == ConfigurationLevels.Application)))
        .ReturnsAsync(appSettingsDocument.Data.ToDictionary());
      connectionMock
      ...
      var sut = _autoFixture.Create<ApplicationSettingsManager>();
      sut.CreateListeners(provider.JsonSettingsToDictionary, provider.ReloadSettings);
      await sut.LoadDocumentSettingsOnChangeAsync(ConfigurationLevels.Machine, "snapshotId");
